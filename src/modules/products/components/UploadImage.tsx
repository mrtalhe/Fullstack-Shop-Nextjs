...
const UploadImage: FC<{ productId: string }> = ({ productId }) => {
  ...
  const handleChangeFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      if (!selectedFile.type.startsWith('image/')) {
        setError('you can just upload Image Please Select a Image File');
        setFile(null);
        return;
      }
      if (selectedFile.size > 2 * 1024 * 1024) {
        setError('Image size should be less than 2MB Please Select another Image File');
        setFile(null);
        return;
      }
  ...
  const handleUpload = async () => {
    if (!file || !productId) {
      setError('please select a valid file and product');
    } else {
